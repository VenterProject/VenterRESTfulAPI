...
class ModelWCView(APIView):
    ...
    def get(self, request):
        ...
        if File.objects.filter(wordcloud_data__icontains = 'wc_output').filter(ckpt_date__gt = prev_week_datetime):
            file_instance = File.objects.filter(wordcloud_data__icontains = 'wc_output').get(ckpt_date__gt = prev_week_datetime)

            temp1 = file_instance.filename
            temp2 = os.path.splitext(temp1)
            custom_input_file_name = temp2[0]
            
            wc_output_json_file_name = custom_input_file_name+'.json'
            results = os.path.join(MEDIA_ROOT, f'{file_instance.organisation_name}/{file_instance.ckpt_date.date()}/wc_output/{wc_output_json_file_name}')
